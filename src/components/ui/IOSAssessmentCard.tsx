...
import { IOSAssessment, IOS_CRITERIA, SOURCE_TIERS } from '@/services/iosFramework';
...
export const IOSAssessmentCard: React.FC<IOSAssessmentCardProps> = ({
  ...
  const calculateSourceVerificationScore = () => {
    const allSources = Object.values(assessment.dimensions).flatMap(d => d.sources);
    ...
    <Card className={`p-6 ${className}`} onClick={onClick}>
      ...
      <div className="mb-6 p-4 bg-blue-50 rounded-lg">
        ...
        <div className="grid grid-cols-5 gap-2 text-xs">
          {[1, 2, 3, 4, 5].map(tier => {
            const tierSources = Object.values(assessment.dimensions).flatMap(d => 
          ...
          <div key={key} className="border rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <div className="text-xs text-gray-500">{IOS_CRITERIA[key as keyof typeof IOS_CRITERIA]?.weight}% weight</div>
              </div>
              ...
              {Object.entries(dimension.subscores).map(([subscoreKey, score]) => (
                <div key={subscoreKey} className="text-center">
                  <div className="text-xs text-gray-500 mb-1">
                    {subscoreKey.replace(/([A-Z])/g, ' $1').trim()}
                  </div>
                        ...
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center">
                            <span className={`inline-block w-3 h-3 rounded-full mr-2 ${
                              source.tier <= 2 ? 'bg-green-400' : 
                              source.tier <= 3 ? 'bg-yellow-400' : 'bg-red-400'
                            }`}></span>
                            <span className="text-xs font-medium text-gray-900">{source.name}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <span className={`px-2 py-1 rounded text-xs font-medium ${SourceVerificationService.getTierColor(source.tier)}`}>
                              Tier {source.tier}
                            </span>
