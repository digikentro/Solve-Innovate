...
def fix_array_schemas(json_schema: Any) -> None:
    """
    Recursively fixes array schemas for OpenAI API compatibility.
    Converts 'prefixItems' (JSON Schema 2020-12 / Zod tuple) and
    list-style 'items' (JSON Schema draft-07 tuple) to a single-schema
    'items' dict, which OpenAI requires for ALL array schemas regardless
    of strict mode.
    ...
    # Convert prefixItems → items (Zod 3.22+, JSON Schema 2020-12)
    prefix_items = json_schema.get("prefixItems")
    if isinstance(prefix_items, list) and not isinstance(json_schema.get("items"), dict):
        unique_schemas: list = []
        for s in prefix_items:
            if s not in unique_schemas:
                unique_schemas.append(s)
        json_schema["items"] = unique_schemas[0] if len(unique_schemas) == 1 else {"anyOf": unique_schemas}
        del json_schema["prefixItems"]

    # Convert list-style items → single-schema items (JSON Schema draft-07)
    ...
    # Recurse into scalar-valued nested schemas
    for key in ("items", "additionalItems", "additionalProperties", "contains", "not"):
        if isinstance(json_schema.get(key), dict):
            fix_array_schemas(json_schema[key])

    # Recurse into named sub-schemas
    for group_key in ("properties", "$defs", "definitions"):
        if isinstance(json_schema.get(group_key), dict):
            for v in json_schema[group_key].values():
                fix_array_schemas(v)

    # Recurse into list-valued sub-schemas
    for list_key in ("anyOf", "allOf", "oneOf"):
        if isinstance(json_schema.get(list_key), list):
            for entry in json_schema[list_key]:
                fix_array_schemas(entry)
...
def ensure_strict_json_schema(
    ...
    *,
    ...
    # arrays
    # Convert prefixItems (JSON Schema 2020-12 tuples) to items for OpenAI compatibility
    prefix_items = json_schema.get("prefixItems")
    if isinstance(prefix_items, list) and "items" not in json_schema:
        # Collect unique schemas from prefixItems
        unique_schemas = []
        for schema in prefix_items:
            if schema not in unique_schemas:
                unique_schemas.append(schema)
        if len(unique_schemas) == 1:
            json_schema["items"] = unique_schemas[0]
        else:
            json_schema["items"] = {"anyOf": unique_schemas}
