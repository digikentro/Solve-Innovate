...
# Supabase — use the anon key, NOT the service-role key.
# The anon key respects Row Level Security policies, preventing writes
# even if this code is somehow misused.
def get_supabase_url_env():
    return os.getenv("SUPABASE_URL")


def get_supabase_key_env():
    return os.getenv("SUPABASE_ANON_KEY")


# Optional: use service-role key for server-side reads in dev/self-hosted setups.
# If enabled, this bypasses RLS, so keep it server-only and never expose to clients.
def get_supabase_service_role_key_env():
    return os.getenv("SUPABASE_SERVICE_ROLE_KEY")


def get_use_supabase_service_role_env():
    return os.getenv("USE_SUPABASE_SERVICE_ROLE")
