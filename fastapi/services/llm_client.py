...
class LLMClient:
    ...
    async def _generate_openai_structured(
        ...
        if depth == 0 and not strict:
            # Even in non-strict mode OpenAI requires valid array schemas (items must be
            # a single schema dict, not prefixItems / list-style items from Zod tuples).
            response_schema = deepcopy(response_schema)
            fix_array_schemas(response_schema)
